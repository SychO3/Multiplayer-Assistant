        ...
        public override void Process(BehaviorState state)
        {
            if (Utils.Sleeping.ShouldSleep(state.GetNumOtherPlayers()) && !Utils.Sleeping.IsSleeping())
            {
                ...
                else if (Game1.currentLocation is FarmHouse)
                {
                    ...
                    ReadyCheckHelper.SetReady("sleep", Game1.player, true);
                ...
                {
                    if (MultiplayerAssistant.Utils.Sleeping.ForceReturnToBed())
                    {
                    ...
                    {
                        // Wait and re-check next tick instead of forced warp
                        state.Sleep();
                    }
                }
            } else if (!Utils.Sleeping.ShouldSleep(state.GetNumOtherPlayers()) && Utils.Sleeping.IsSleeping())
            {
                ...
                else
                {
                    ...
                    ReadyCheckHelper.SetReady("sleep", Game1.player, false);
                    state.CancelSleep();
                }
            }
            else
