...
namespace MultiplayerAssistant.MessageCommands
{
    internal class ServerCommandListener
    {
        ...
        private void chatReceived(object sender, ChatEventArgs e)
        {
            ...
            // ChatBox.privateMessage may change across game versions; 3 is the private message chat kind.
            if( (3 == e.ChatKind) && (moveBuildPermissionCommand.Any(tokens[0].Equals)) )
            {
                ...
                if (config.LockCabinMove)
                {
                    chatBox.textBoxEnter($"/message {sourceFarmer} Error: MoveBuildPermission is locked by server config.");
                }
                else if (moveBuildPermissionParameter.Any(newBuildPermission.Equals))
                {
